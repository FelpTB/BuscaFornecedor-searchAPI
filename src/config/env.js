...
export function getAuthMode() {
  const mode = (process.env.AUTH_MODE || "off").trim().toLowerCase();
...
export function validateEnv({ soft = false } = {}) {
  ...
  if (getAuthMode() === "api_key" && !process.env.AUTH_API_KEYS?.trim()) {
    warnings.push("AUTH_MODE=api_key sem AUTH_API_KEYS — todas as requests autenticadas falharão");
  }
